...
func _ready():
	...
	EventBus.camera_bounds_updated.connect(__camera_bounds_updated_handler)
...
func __camera_panned_handler(pan_direction: Vector2):
	if is_in_tween == true:
		return
	var to_be_x = clampf(position.x + pan_direction.x * pan_sensitivity/zoom.x, min_x, max_x)
	var to_be_y = clampf(position.y + pan_direction.y * pan_sensitivity/zoom.y, min_y, max_y)
...
func __camera_bounds_updated_handler(max_x_mg, min_x_mg, max_y_mg, min_y_mg):
	max_x = Global.Util.global_pos_at(max_x_mg)
	min_x = Global.Util.global_pos_at(min_x_mg)
	max_y = Global.Util.global_pos_at(max_y_mg)
	min_y = Global.Util.global_pos_at(min_y_mg)
